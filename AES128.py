...
size = 4
AES_Sbox = (0x63, 0x7C, 0x77, 0x7B, 0xF2, 0x6B, 0x6F, 0xC5, 0x30, 0x01, 0x67,
...
column_Matrix = np.array([[0x02, 0x03, 0x01, 0x01], [0x01, 0x2, 0x03, 0x01],
...
AES_SboxInverse = (
    ...
    0x55, 0x21, 0x0C, 0x7D)
AES_SboxInverse_array = np.array(AES_SboxInverse, dtype=np.uint8).reshape(
    (16, 16))
...
def AES_Encrypt():
  ...
  for row in range(4):
    print(state[0, row] + " " + state[1, row] + " " + state[2, row] + " " +
          state[3, row] + " ")

  print("\nKey 1 in Binary:")
  for row in range(4):
    print(keyBytesArr[0, row] + " " + keyBytesArr[1, row] + " " +
          keyBytesArr[2, row] + " " + keyBytesArr[3, row] + " ")
